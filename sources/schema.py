...
class Schema:
    ...
    def clone(
        self,
        *,
        cmap: CloneDefault | int | None = CLONE_DEFAULT,
        path: CloneDefault | Shape = CLONE_DEFAULT,
        size: CloneDefault | float = CLONE_DEFAULT,
        joining_type: CloneDefault | Type = CLONE_DEFAULT,
        side_bearing: CloneDefault | float = CLONE_DEFAULT,
        y_min: CloneDefault | float | None = CLONE_DEFAULT,
        y_max: CloneDefault | float | None = CLONE_DEFAULT,
        child: CloneDefault | bool = CLONE_DEFAULT,
        ...
        cps: CloneDefault | Sequence[int] | None = CLONE_DEFAULT,
        original_shape: CloneDefault | type[Shape] | None = CLONE_DEFAULT,
        _anchors: CloneDefault | set[str] | None = CLONE_DEFAULT,
    ) -> Self:
        return type(self)(
    ...
    def feature_suffix(self) -> str:
        """A glyph name component inspired by feature tags.

        The glyph should be similar to glyphs typical of the returned
        tag(s) but need not literally be generated by lookups with those
        features.

        Returns:
            A string of zero or more concatenated feature tags, each
            preceded by a period.
        """
        if self.cmap is None and (cps := self.cps):
            if cps == (0x2044,):
                return '.frac' if self.y_max is not None else '.afrc'
            if self.y_min is not None and self.y_max is not None and unicodedata.category(chr(cps[0])) == 'Nd':
                if self.y_min < 0:
                    return '.subs' if self.side_bearing == DEFAULT_SIDE_BEARING else '.dnom.afrc'
                elif self.y_max > CAP_HEIGHT:
                    return '.sups' if self.side_bearing == DEFAULT_SIDE_BEARING else '.numr.afrc'
                elif self.y_min == 0:
                    return '.dnom'
                else:
                    return '.numr'
