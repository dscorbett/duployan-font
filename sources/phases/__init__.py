...
def _add_rule(
    ...
    local_output_schemas: MutableSet[schema.Schema],
    ...
    Args:
        autochthonous_schemas: All the schemas ever created in the
            current phase, not including the current iteration.
        output_schemas: All the schemas that could possibly appear in
            the glyph stream after the current phase. This function may
            add or remove schemas to or from the set.
        local_output_schemas: The subset of `output_schemas` created in
            the current phase. This function may add schemas to the set.
    ...
    def remove_unconditionally_substituted_schemas() -> None:
        ...
        non-contextual rule of a required GSUB lookup, the input of
        `rule` contains only one element which either is or contains
        that schema, and no rule in the lookup uses (or could possibly
        use, subject to later modifications to named lookups in the same
        ...
        for schema_to_remove in unconditionally_substituted_schemas:
            if schema_to_remove not in local_output_schemas:
                ...
                if isinstance(output, str):
                    ...
                    for o in classes[output]:
                        if o not in output_schemas:
                            ...
                            local_output_schemas.add(o)
                    if must_freeze:
                        classes[output].freeze()
                        froze = True
                else:
                    output_schemas.add(output)
                    local_output_schemas.add(output)
            return froze
...
def run_phases(
    ...
    for phase_index, phase in enumerate(phases, start=schema.CURRENT_PHASE_INDEX + 1):
        ...
        local_output_schemas: MutableSet[schema.Schema] = set()
