...
_NOTO_EXCLUSIONS: Final[AbstractSet[int]] = {
    ...
    # Brandt’s Danish adaptation is not fully supported in Unicode.
    0x0326, 0x1AB7,
...
def initialize_schemas(charset: Charset, light_line: float, stroke_gap: float) -> Collection[Schema]:
    ...
    dotted_cross = Complex([*plus.instructions, (plus.instructions[1].size * (1 - 2 / 3), Line(180), True), (plus.instructions[1].size * 2 / 3, Line(90), True), (0, h), (plus.instructions[1].size * 2 * 2 / 3, Line(180), True), (0, h), (plus.instructions[1].size * 2 * 2 / 3, Line(270), True), (0, h), (plus.instructions[1].size * 2 * 2 / 3, Line(0), True), (0, h)])  # type: ignore[misc, union-attr]
    ...
    left_double_parenthesis = Complex([*left_parenthesis.instructions, *[op if callable(op) else (op.size, op.shape.as_reversed(), True) for op in reversed(left_parenthesis.instructions)], parenthesis_horizontal_space, *left_parenthesis.instructions])  # type: ignore[attr-defined, misc]
    right_double_parenthesis = Complex([*right_parenthesis.instructions, *[op if callable(op) else (op.size, op.shape.as_reversed(), True) for op in reversed(right_parenthesis.instructions)], parenthesis_horizontal_space, *right_parenthesis.instructions])  # type: ignore[attr-defined, misc]
    ...
    long_wiggly_dash = Complex([*wiggly_dash.instructions] * 2)
    wiggly_rising_diagonal = Complex([(1, Curve(360 - 60 + 45, 60 + 45, clockwise=False)), (1, Curve(60 + 45, 360 - 60 + 45, clockwise=True))] * 6)
    wiggly_falling_diagonal = Complex([(1, Curve(360 - 60 - 45, 60 - 45, clockwise=False)), (1, Curve(60 - 45, 360 - 60 - 45, clockwise=True))] * 6)
    ...
    wi = Wi([(4, Circle(180, 180, clockwise=False)), lambda c: c.clone(clockwise=None, ignorable_for_topography=False), (5 / 3, m)])
    wei = Wi([(4, Circle(180, 180, clockwise=False)), lambda c: c.clone(clockwise=None, ignorable_for_topography=False), (1, m), lambda c: c.clone(clockwise=not c.clockwise), (1, n)])
    ...
    high_line = SeparateAffix([(0.7, Line(0))])
    ...
    high_long_grave = SeparateAffix([*high_grave.instructions, *high_line.instructions])
    ...
    low_arrow = SeparateAffix([(1, Line(360 - 23.9)), (1, Line(180 + 23.9))], low=True)
    ...
    down_step = InvalidStep([*dotted_square, (444, Space(0)), (749, Space(90)), (1.184, Line(270)), (0.32, Line(130)), (0.32, Line(310), True), (0.32, Line(50))], up=False)
    up_step = InvalidStep([*dotted_square, (444, Space(0)), (157, Space(90)), (1.184, Line(90)), (0.32, Line(230)), (0.32, Line(50), True), (0.32, Line(310))], up=True)
    ...
    new_moon = Complex([*[(op[0] * full_moon.instructions[0].size * RADIUS / LINE_FACTOR, *op[1:]) for i in range(8) for op in [(0.993 * math.sqrt(1 - (((i + 1) - (8 + 1) / 2) / ((8 + 1) / 2)) ** 2), Line(90), True), (1.993 * math.sqrt(1 - (((i + 1) - (8 + 1) / 2) / ((8 + 1) / 2)) ** 2), Line(270)), (math.sqrt(1 - (((i + 1) - (8 + 1) / 2) / ((8 + 1) / 2)) ** 2), Line(90), True), (2 / (8 + 1), Line(0), True)]], (full_moon.instructions[0].size * RADIUS / LINE_FACTOR * math.sqrt(2), Line(225), True), *full_moon.instructions])  # type: ignore[list-item, misc, operator, union-attr]
    line = Line(0)
    ...
    schemas = [
        ...
        Schema(0x002B, plus, 1, Type.NON_JOINING, y_min=CAP_HEIGHT / 2 - light_line / 2 - plus.instructions[1].size * LINE_FACTOR),  # type: ignore[misc, union-attr]
        ...
        Schema(0x00B7, h, 0, Type.NON_JOINING, y_min=CAP_HEIGHT / 2 - light_line * Dot.SCALAR ** h.size_exponent / 2),  # type: ignore[misc]
        ...
        Schema(0x0326, comma, 1, anchor=anchors.BELOW),
        ...
        Schema(0xE01A, long_wiggly_dash, 2, Type.NON_JOINING),
        Schema(0xE01B, wiggly_rising_diagonal, 2, Type.NON_JOINING, y_min=BRACKET_DEPTH, y_max=BRACKET_HEIGHT),
        Schema(0xE01C, wiggly_falling_diagonal, 2, Type.NON_JOINING, y_min=BRACKET_DEPTH, y_max=BRACKET_HEIGHT),
        ...
        Schema(0x1BC61, s_t, 3.2, can_lead_orienting_sequence=True, shading_allowed=False),
        Schema(0x1BC62, s_n, 3.2, Type.ORIENTING, shading_allowed=False),
        Schema(0x1BC63, t_s, 3.2, can_lead_orienting_sequence=True, shading_allowed=False),
    ...
    for small_digit_cp in [
        ...
        schema = next((s for s in schemas if s.cmap == base_digit_cp), None)
        if schema is None:
            continue
